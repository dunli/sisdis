        ...
        case POST:
           {
                ...
                if(s2->compare("/services.php")==0)
                {
                    ...
                    // caso checkbox pruebaclase
                    if(getFromPost(postLine, "pruebaclase")){

                        pruebaStub *pclase = new pruebaStub();
                        pclase->holaMundo();
                        int resultado = pclase->suma(1,3);
                        char *headerHtml = "<!DOCTYPE html><html><head></head><body><p> El resultado de tu suma es: <b>";
                        char *footerHtml = "</b></p></body></html>";


                        std::cout<< "resultado: "<<resultado<<std::endl;

                        //TODO: Añadir el resultado a resultado.html
                        std::ofstream myfile;
                            myfile.open ("../html_dir/resultado.html");
                            myfile << "<!DOCTYPE html><html><head></head><body>";

                            //add some html content
                            myfile << "<p> El resultado de la suma es: <b>"<<resultado<<"</b></p>";

                           //ending html
                            myfile << "</body></html>";
                            myfile.close();

                        // enviamos la pagina html con el resultado de la operación
                    ...
                    // caso checkbox remotefile
                   else if(getFromPost(postLine, "remoteFile")){
                        remotefile_stub* file = new remotefile_stub();
                        char *buff = NULL;
                        unsigned long int buffleng = 0;
                        file->readfile("/home/ubuntu/prueba.txt",&buff, &buffleng);
                        std::cout<<"Fichero Leido: "<< buff<<std::endl;

                        //TODO: Añadir el buff (cadena de texto del .txt) a resultado.html
                        ...
                        std::ofstream myfile;
                            ...
                            //add some html content
                            myfile << "<p> El archivo contiene el siguiente texto: <b>"<<buff<<"</b></p>";
...
bool httpServer::validatePassword(char* username,char* password)
{
    ...
    try
    {
      ...
      con = driver->connect("tcp://127.0.0.1:3306", "paco", "pipas");
      /* Connect to the MySQL database */
      con->setSchema("userLogin");
